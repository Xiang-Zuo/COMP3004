...
public class gameControl : MonoBehaviour
{
    ...
    private List<Card>[] otherPlayersCardsInQuest = new List<Card>[4];
    ...
    void Start()
    {
        print("path----" + Directory.GetCurrentDirectory());
        //setHands_players();
        CURRENTPLAYERNUM = 4;
    ...
    void drawACard(Player aPlayer, string getTheCard, string discardTheCard, string allyOrAmour)
    {
        ...
        if (theCard == null)
        {
            gameLog.error("failure to add " + getTheCard + " to player's hand");
        }
        else
        {
            ...
            else if (currentView == 2)
            {
                if (aPlayer.getName() == p1.getName())
                {
                    getCard(p1, 5, theCard.getName(), 3);
                }
                if (aPlayer.getName() == p2.getName())
                {
                    getCard(p2, 5, theCard.getName(), 4);
                }
                if (aPlayer.getName() == p3.getName())
                {
                    getCard(p3, 5, theCard.getName(), 1);
                    sortCard(p3);
                }
                if (aPlayer.getName() == p4.getName())
                {

                    getCard(p4, 5, theCard.getName(), 2);
                }
            }
            else if (currentView == 3)
            {
                if (aPlayer.getName() == p1.getName())
                {
                    getCard(p1, 5, theCard.getName(), 2);
                }
                if (aPlayer.getName() == p2.getName())
                {
                    getCard(p2, 5, theCard.getName(), 3);
                }
                if (aPlayer.getName() == p3.getName())
                {
                    getCard(p3, 5, theCard.getName(), 4);
                }
                if (aPlayer.getName() == p4.getName())
                {
                    getCard(p4, 5, theCard.getName(), 1);
                    sortCard(p4);
                }
            }
    ...
    public void askIfJoin(int stat)
    {
        ...
        otherPlayersCardsInQuest[0] = a;
        otherPlayersCardsInQuest[1] = b;
        otherPlayersCardsInQuest[2] = c;
        otherPlayersCardsInQuest[3] = d;
        if (joinCount < 2)
        {
            if (stat == 0)
            {
        ...
        else
        {
            ...
            drawACard(players[currentView], null, null, null);
            /**call something here
			 * 
    ...
    public void discardThis()
    {
        ...
        {
            cards.Add(GameObject.Find(players[currentView].getHand()[i].getName()));
            //cards [i].GetComponent<TouchCard> ().activateCard ();
            if (cards[i].GetComponent<TouchCard>().isChosen())
            {
                theDiscardCard = discardExtraCard(players[currentView], cards[i].name);
    ...
    void checkValidOtherPlayersInQuestPlayCards(int round)
    {
        ...
        int playerNow = 0;
        bool isFinish = false;
        //get the position of player in playerInQuest(List<Player>)
        for (int i = 0; i < playerInQuest.Count; i++)
        {
            if (playerInQuest[i].getName() == players[currentView].getName())
            {
                playerNow = i;
            }
        }
        ...
        {
            CheckPassOtherPlayersInQuestPlayCards(round, otherPlayersCardsInQuest);
            roundNow++;
        ...
        for (int i = 0; i < 4; i++)
        {
            if (!playerInQuest.Contains(players[currentView]) && !isFinish)
                switchView();
            else
            {
                drawACard(players[currentView], null, null, null);
                print("drawacard" + players[currentView].getHand().Count);
                break;
            }
        }
    }

    void CheckPassOtherPlayersInQuestPlayCards(int round, List<Card>[] playerListArray)
    {
        int atk = 0;
        int playerNum = playerInQuest.Count;

        for (int i = 0; i < playerNum; i++)
        {
            if (playerListArray[i].Count == 0)
                atk = playerInQuest[i].getAtk();
            else
                atk = calculatePlayerATK(questNow, playerListArray[i], playerInQuest[i]);
            print("--" + sponsorStageATK[round - 1]);
            ...
            if (atk < sponsorStageATK[round - 1])
            {
                ...
                print(playerInQuest[i].getName() + " did not pass the quest");

            }
            else
            {
                gameLog.info(playerInQuest[i].getName() + " passes the quest");
                print(playerInQuest[i].getName() + " passes the quest");
            }
        }
        for (int i = 0; i < playerListArray.Length; i++)
        {
            playerListArray[i].Clear();
        }
        for (int i = 0; i < playerInQuest.Count; i++)
        {
